...
import { fn, col, Model } from 'sequelize';
import { asyncHandler, needAuth } from '../middlewares';
import { Board, Comment, Article } from '../db';
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    const boardId = Number(req.query.boardId as string);
    const allowedLimits = [5, 15, 30, 50];
    let limit = Number((req.query.limit as string) || 15);
